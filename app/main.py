...
WINDOW_HEIGHT = GRID_HEIGHT * TILE_SIZE + BOTTOM_BAR_HEIGHT + DUMP_AREA_HEIGHT
FONT_SIZE = 36
...
class PlayerBar:
    ...
    def draw(self):
        y = GRID_HEIGHT * TILE_SIZE + DUMP_AREA_HEIGHT
...
def main():
    ...
    while running:
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if event.button == 1:  # Left click
                    x, y = event.pos
                    if y > GRID_HEIGHT * TILE_SIZE + DUMP_AREA_HEIGHT:
                        index = x // TILE_SIZE
                ...
                if dragging:
                    ...
                    if dump_area.is_in_area((x, y)):  # Dump the tile
                        player_bar.add_tiles(pool.dump_tiles(dragged_tile))
                    else:  # Place on the board
        ...
        if dragging and dragged_tile:
            ...
            pygame.draw.rect(screen, TILE_COLOR, (mouse_x - drag_offset_x, mouse_y - drag_offset_y, TILE_SIZE, TILE_SIZE))
            text = board.font.render(dragged_tile, True, TEXT_COLOR)
            text_rect = text.get_rect(center=(mouse_x - drag_offset_x + TILE_SIZE // 2, mouse_y - drag_offset_y + TILE_SIZE // 2))
